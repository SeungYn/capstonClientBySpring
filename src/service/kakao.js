export default class KakaoService {
  ...
  //스프링서버 /user/restaurants
  async getAllMap() {
    const data = await this.http.fetch(`/user/restaurants`, {
      method: 'GET',
  ...
  addEventListener(marker, event, action) {
    this.kakao.maps.event.addListener(marker, event, (e) => {
      console.log(e);
      action(e);
    });
  }

  removeEventListener(marker, event, action) {}
