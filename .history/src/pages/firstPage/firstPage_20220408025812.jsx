...
const FirstPage = ({ kakao, kakaoService }) => {
  ...
  const [refresh, setRefresh] = useState({ st: 'ff' });
  ...
  const onNotClickedEvent = () => {
    console.log('kkkk');
    for (let i = 0; i < restaurants.length; i++) {
      kakao.maps.event.addListener(makers[i], 'click', () => {
        console.log(4444);
      });
    }
  };
  ...
  }, [currentLocation, currentError, refresh]);
  ...
  return (
    <section className={styles.container}>
      <div ref={reizeContainerRef} className={styles.map__group}>
        ...
        <button onClick={onNotClickedEvent} className={styles.notClickEvent}>
          test
        </button>
      </div>
