...
const MyParty = ({ partyService }) => {
  ...
  const onReady = useCallback(() => {
    partyService.partyReady(party.restaurantId).then((data) => {
      console.log(data);
  ...
  const outParty = () => {
    partyService.outParty(party.id).then((data) => {
      return nav('/');
    });
  };
      ...
      {party && (
        ...
        <li className={styles.partyInfo__address}></li> */}
            ...
            <li className={styles.partyInfo__state}>
              파티상태 :{' '}
              {`${party.matchingStatus == 'NON_MATCHED' ? '대기중' : '시작'}`}
            </li>
          </ul>
          <div className={styles.report}>
            {party.matchingStatus == 'NON_MATCHED' ? (
              <button
