...
const Chat = ({ chatService, kakaoService }) => {
  ...
  const movePosition = (posSocketData, flag) => {
    if (!flag) {
      return;
    }
    let { location, longitude, nickName } = posSocketData;
    nickName = posSocketData.nickname.toString();
  ...
  //지도와 실시간 위치
  useEffect(() => {
    if (!connectFlag || !party || !agree || !firstLocation) {
      return;
    }
    ...
    setPreMarker(marker);
  }, [chatService, connectFlag, party, agree, firstLocation]);

  //실시간 위치
  useEffect(() => {
    let flag = true;
    console.log('움직임');
    if (!partyid || !myName || !mapSwitch || !flag) {
      return;
    }
  ...
  }, [location, party, myName, mapSwitch]);
  return (
