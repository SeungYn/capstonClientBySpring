...
const Login = ({
  ...
  const onChange = (event) => {
    ...
    switch (name) {
      ...
      case 'username':
        if (signup) {
          idDuplicateVerification(value).then(
            (data) => data && setduplicateError(true)
          );
  ...
  const submit = (e) => {
    ...
    if (signup) {
      onSignUp(nickname, username, password, email, sex, university, dept, sno)
        .then((data) => {
          if (data) {
            setSignup(false);
            setUsername('');
      ...
      onLogin(username, password).catch(console.log);
    }
    console.log(1);
  };
